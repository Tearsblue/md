...
{
    // printf("lSpped : %ld res : %.5f\n", lSpeed, ((double)lSpeed*(long)RB.nNum)/(long)RB.nDenom);
...
double RBAngRatio2RPM(long lAngRatio)
{
    ...
    else if(Com.nAngResol == 1)
    {
        ...
        dResult = (dAngRatio*(double)Com.nWheelLength*(double)Com.nGearRatio)/(6*(double)Com.nDiameter);
    }
